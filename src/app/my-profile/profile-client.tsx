              ...
              <div className={`flex gap-1 overflow-x-auto overscroll-x-contain p-1 ${imgs.length === 1 ? "justify-center" : ""}`}>
                {imgs.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="relative flex-shrink-0 w-60 h-72 rounded-xl overflow-hidden block cursor-pointer">
                    <Image src={url} alt={`${c.full_name} ${i + 1}`} fill className="object-contain" unoptimized />
                  </a>
                ))}
              </div>
            </div>
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          ...
          <EditSection title="פרטים אישיים">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <EditMultiSelect name="military_service" label="שירות" options={["שירות לאומי", "קרבי", "קבע", "צבא", "הסדר", "עתודה", "ללא"]} defaultValues={c.military_service ? c.military_service.split(",").map((s: string) => s.trim()) : []} />
            </div>
          </EditSection>
...
function EditMultiSelect({ name, label, options, defaultValues }: {
