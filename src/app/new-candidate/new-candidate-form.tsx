...
export function NewCandidateForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Signal whether this is for another person
    if (isLoggedIn && forOther) {
      formData.set("for_other", "1");
    }
    ...
    // Signal ambassador mode so server can validate and create ambassador auth account
    if (isAmbassadorMode) {
    ...
    try {
      ...
      setError(
        selectedGender === "נקבה"
          ? "אירעה שגיאה בשמירת הפרטים. ייתכן שהתמונות שהעלית גדולות מדי — נסי תמונות קטנות יותר או פחות תמונות ונסי שוב."
          : "אירעה שגיאה בשמירת הפרטים. ייתכן שהתמונות שהעלית גדולות מדי — נסה תמונות קטנות יותר או פחות תמונות ונסה שוב."
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {isLoggedIn ? "חזרה לפרופיל" : "חזרה להתחברות"}
            </Link>
          </div>
        </div>
      </header>
          ...
          {isLoggedIn && !inviteToken && (
            <Section title="סוג הרשמה">
              <div className="flex gap-3">
                ...
                >
                  <div className="font-bold mb-0.5">הרשמה לעצמי</div>
                  <div className="text-[11px] opacity-70">אני המועמד/ת</div>
                </button>
                <button
                  type="button"
              ...
              <div className="flex flex-wrap gap-3 mt-2">
                {images.map((file, i) => (
                  <div key={i} className="relative w-24 h-24 rounded-xl overflow-hidden border border-gray-200 group">
                    ...
                    <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 w-6 h-6 rounded-full bg-red-500 text-white flex items-center justify-center text-xs shadow-md">X</button>
                  </div>
                ))}
                {images.length < 3 && (
          ...
          <Section title="אישור תנאים">
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                name="accept_terms"
                required
                className="mt-1 w-4 h-4 rounded border-gray-300 text-sky-500 focus:ring-sky-400 flex-shrink-0"
              />
